"""Spline-to-node-boundary clipping pipeline.

See: /lib/common/splines.c @ 65

Takes raw cubic-Bezier control points produced by a router and clips
the first / last segments to the tail / head node boundaries.  All
functions here are pure geometry — no Graph / LayoutEdge / LayoutNode
coupling — so any layout engine that produces bezier splines can use
them.

Pipeline:

1. :func:`bezier_clip` — binary-search sub-curve intersection with the
   node boundary via a shape-specific inside-test.
2. :func:`shape_clip0` — coordinate-frame wrapper (graph → node-local
   → graph).
3. :func:`shape_clip` — auto-detect which end is inside.
4. :func:`clip_and_install` — whole-spline driver that calls the
   above for both endpoints and strips degenerate segments.

Helper: :func:`conc_slope` computes the mean slope at a concentrator
node (used by Graphviz when multiple edges are bundled through one
node).

``dot/clip.py`` re-exports every symbol for back-compat.
"""
...
def clip_and_install(
    ...
    *,
    ...
    Takes the raw control points produced by a router and clips the
    first cubic segment to the tail node boundary and the last cubic
    segment to the head node boundary.  Also strips any degenerate
    (zero-length) segments at either end.
...
def conc_slope(node_x: float, node_y: float,
    ...
    """Mean slope at a concentrator node.
