...
### §7.x — `-Tplain` output format (CLOSED 2026-05-09)

`gvcli.py -Tplain` now emits the canonical Graphviz plain text
format (`graph / node / edge / stop` lines, inches, math-y
coordinates), matching what C `dot -Tplain` produces.  JSON
output stays available under `-Tjson` / `-Tjson0`.

Implementation: `gvpy/render/plain_renderer.py` (~190 lines).
16 new tests covering token quoting, header line, default
attrs, edge point counts, optional edge labels, and end-to-end
CLI dispatch.
