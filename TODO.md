...
| D6 | Phase 3 position lacks hard keep-out constraints for virtuals vs. non-member cluster y-bands | `position.py` — attempted 2026-04-20 as pre-rankdir virtual push + `recover_slack` guard, reverted: corpus crossings unchanged (push helps individual virtuals but phase-4 ``recover_slack`` keeps re-centring to corridor midpoint; shifting crossings around without net improvement).  **Real fix candidate: "corridor-carve" — replace `rank_box(rank)` with `rank_box_gapped(rank, edge)` that returns a list of rectangles covering the rank strip MINUS non-member cluster bboxes, so ``routesplines``/``routepolylines`` physically can't thread through a cluster interior.  Sketch in commit 524f1a8 discussion: ~200-300 lines touching `completeregularpath` / `routespl` / `maximal_bbox` / `rank_box`; likely takes D4 residual 40 → ~10 but each remaining pathological case becomes D5's responsibility.  Risk: touches phase-4 internals that every currently-passing spline test depends on — gate behind `GVPY_CLUSTER_CARVE=1` and measure before flipping default.** Defer until D5 is tackled alongside it. | compounds D4 |
