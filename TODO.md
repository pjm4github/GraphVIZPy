...
| D7 | Font metrics are Times-Roman AFM, not the rendering font | Layout-side already uses TNR via ``text_width_system`` (tkinter) for record port sizing.  Render-side: 2026-04-20 emits the ANTLR-parsed ``record_fields`` tree on the layout-output node dict (``record_tree``) and svg_renderer consumes it in place of its hand-written ``_parse_record_label`` — single-parser consistency between layout and render.  ``_parse_record_label`` kept as fallback for older node dicts.  The specific 1332 port.order=99-vs-42 gap is **not** yet closed — traces to a compass-angle / rankdir rotation divergence in ``RecordField.port_fraction``, not the text-width metric.  That's separate debugging; file under "record-port compass rotation" next pass. | record-node mincross divergence only |
