...
Parse a DOT file and run the layout::
    ...
    from pycode.dot_layout import DotLayout
...
To render the result as SVG::
    ...
    from pycode.svg_renderer import render_svg, render_svg_file
...
class DotLayout:
    ...
    def _scan_clusters(self, g: Graph):
        for sub_name, sub in g.subgraphs.items():
            if sub_name.startswith("cluster"):
                ...
                margin = float(margin_str) * 72.0 if margin_str else 8.0
    ...
    def _record_size(self, label: str, fontsize: float, char_w: float) -> tuple[float, float]:
        ...
        Example: "a|{b|c}|d" has 3 top-level fields; the middle one has 2 sub-fields.
        """
        fields = _parse_record_fields(label)
        if not fields:
            return self._MIN_WIDTH, self._MIN_HEIGHT

        # Total width = sum of field widths + padding between fields
        total_w = 0.0
        max_h = fontsize * 1.2 + self._V_PAD
        for f in fields:
            text = f["text"]
            fw = len(text) * char_w + self._FIELD_PAD * 2
            fw = max(fw, 20.0)  # minimum field width
            total_w += fw
            if f["children"]:
                # Vertical sub-fields: height = sum of children heights
                child_h = len(f["children"]) * (fontsize * 1.2 + 4.0)
                max_h = max(max_h, child_h + self._V_PAD)
    ...
    def _phase3_position(self):
        ...
        # X coordinates: try network simplex, fall back to simple placement
    ...
    def _ns_x_position(self) -> bool:
        ...
        # Alignment edges: connected nodes pulled toward vertical alignment
        for le in self.ledges:
            if le.tail_name in self.lnodes and le.head_name in self.lnodes:
                w = le.weight * 2
    ...
    def _compute_xlabel_positions(self):
        """Compute positions for xlabel, headlabel, and taillabel attributes."""
        # Node xlabels: positioned outside the node (below-right by default)
        for name, ln in self.lnodes.items():
            if ln.virtual or not ln.node:
                continue
            xlabel = ln.node.attributes.get("xlabel", "")
            if xlabel:
                ln.node.attributes["_xlabel_pos_x"] = str(round(ln.x + ln.width / 2 + 4, 2))
                ln.node.attributes["_xlabel_pos_y"] = str(round(ln.y + ln.height / 2 + 12, 2))
        ...
        for le in all_edges:
            ...
            if headlabel:
                # Position near the head end of the edge
                hp = le.points[-1]
                le.edge.attributes["_headlabel_pos_x"] = str(round(hp[0] + 8, 2))
                le.edge.attributes["_headlabel_pos_y"] = str(round(hp[1] - 8, 2))
            if taillabel:
                # Position near the tail end of the edge
                tp = le.points[0]
                le.edge.attributes["_taillabel_pos_x"] = str(round(tp[0] + 8, 2))
                le.edge.attributes["_taillabel_pos_y"] = str(round(tp[1] + 8, 2))
    ...
    def _phase4_routing(self):
        ...
        if use_bezier:
            all_edges = [le for le in self.ledges if not le.virtual] + self._chain_edges
            for le in all_edges:
                if le.points and len(le.points) >= 2:
