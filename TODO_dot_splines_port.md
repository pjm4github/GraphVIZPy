...
| 1124 | `make_flat_adj_edges` | `flat_edge.make_flat_adj_edges` | partial (B) | E+.2 | Ported 2026-04-18 as option **E+.2-B**: dispatcher + port-aware attach points via `port_point()`, no clone. Covers pure-compass ports on non-record nodes. Record-field ports and non-compass qualifiers emit `UnsupportedPortRoutingWarning` pointing at E+.2-**A** (faithful recursive clone + re-layout + spline transform) which remains deferred. |
